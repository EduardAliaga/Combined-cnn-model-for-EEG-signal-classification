...
def evaluate_one_model(model, test_x, test_y):
    ...
    model_evaluation_dict["confusion_matrix"] = conf_matrix
...
with logger.catch(onerror=lambda _: sys.exit(1)):
    for seed in range(args.seeds):
        ...
        if args.tiny:
            ...
            test_class0_idx = np.where(test_y == 0)[0][:80]
            test_class1_idx = np.where(test_y == 1)[0][:20]
        ...
        for name, model_cls, model_kwargs in NAME_MODEL_KWARGS:
            ...
            logger.info(model_evaluation_dict["confusion_matrix"])
