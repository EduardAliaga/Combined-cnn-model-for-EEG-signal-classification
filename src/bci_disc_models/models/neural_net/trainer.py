...
class Trainer:
    def __init__(self, model, datamodule: Datamodule, lr: float, results_dir: Path, device: torch.device, tqdm_pos=0):
        ...
        self.acc_metric = Accuracy(task="multiclass", num_classes=self.n_classes).to(device)
    ...
    def __call__(self, epochs: int):
        ...
        with open(self.results_dir / "train_epoch_losses.txt", 'w') as f:
            for loss in self.train_epoch_losses:
                f.write(f"{loss}\n") 
        with open(self.results_dir / "val_epoch_losses.txt", 'w') as f:
            for loss in self.val_epoch_losses:
                f.write(f"{loss}\n")        
        return self.metrics
    ...
    def _run_epoch(self, desc, loader, optim=None):
        ...
        if desc == "train":
            self.train_epoch_losses.append(avg_loss)  # Append the average training loss for this epoch to the list
        elif desc == "val":
